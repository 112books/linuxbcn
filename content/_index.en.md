---
title: "Digital identity with criteria"
subtitle: "Web development, custom applications and digital infrastructure. Free software, no complications."
description: "LinuxBCN — technology consultancy in Barcelona for artists, cultural collectives and small businesses. Free software, data sovereignty, 25 years of experience."
lastmod: "2026-06-25"
draft: false
---

## Most digital problems aren't technical

They're about structure.

You have a website you can't update because it depends on an agency that takes three weeks to reply.
Your Instagram has 2,000 followers but your domain expires next month.
You use big-tech tools because everyone does,
but your communications sit on servers you don't control and can't audit.
Your contact form sends data to an American server you've never heard of.

Often, the problem isn't that you lack technology.
It's that you have too much technology you don't understand,
don't control, and couldn't change even if you wanted to.

---

## What we do

We accompany artists, cultural collectives and small businesses
in building a coherent, free and sustainable digital presence.

Concretely:

{{< serveis-en >}}

We don't make catalogue websites.
We don't apply templates.
We don't recommend trendy technology.

We listen, understand the situation, and design the system that makes sense for each project.

---

## Free software isn't ideology. It's practice.

When you use free tools, the code is public.
Someone has checked whether it does what it says it does.
Someone has verified it doesn't send your data to places you haven't authorised.

When you use closed tools, you have to trust.
And trust is expensive when what's at stake is your relationship with your audience,
your internal communications, or your clients' data.

We have seen cultural projects lose years of work
because a platform changed its terms overnight.
We have seen musicians without access to their own fan base
because it lived in an account they didn't control.
We have seen collectives paying indefinitely for tools they didn't need
but didn't know how to remove.

Digital sovereignty isn't a luxury. It's basic hygiene.

---

## How we do it

LinuxBCN has existed since 2001. Twenty-five years accompanying musicians, cultural spaces, independent publishers, neighbourhood collectives and values-driven small businesses. Each project is different. The approach is the same:

{{< proces-en >}}

We work from **Nau Bostik**, La Sagrera — a self-managed sociocultural space that is also the best context for understanding the projects that trust us with their digital presence.

---

## Who we work with in Barcelona and Catalonia

{{< perfils-en >}}

If your project doesn't fit exactly into any of these, let's talk anyway. Every situation is different.

---

## Real projects

The best we can show is what we've done:

→ [See projects](/projectes/)

---

{{< testimoni "I love that the Bratia website practically runs itself." "Ivan Kovačević, Bratia" >}}

{{< testimoni "We had a tool built in Access by a volunteer. We used to spend hours processing data for our annual grant reports. Joan developed a tailored Drupal solution and we went from hours to minutes — exactly what we needed." "Andrea, director of 9 Barris Acull" >}}

{{< testimoni "The website is better than the restaurant." "Sachie, Machiroku" >}}

---

→ [See solutions by profile](/solucions/)
